...
use forge_pen_test::{FsrtRemoteConfig, MintError};

use crate::Result as AppResult;

type Result<T> = std::result::Result<T, MintError>;
...
pub struct HarvestConfig {
    ...
    pub password: String,
    /// Tenant site where the cookie is minted.
    pub site_url: String,
...
impl HarvestConfig {
    /// Build a [`HarvestConfig`] from `fsrt-remote.toml` plus the password.
    ///
    /// - `username`, `headed`, verify-wait come from `[cookie]`.
    /// - `site_url` comes from the shared `site` setting.
    /// - `output` comes from `auth.raw_cookie_file` (the file the token loader
    ///   later reads), so mint-and-consume agree on one path.
    pub fn from_remote_config(config: &FsrtRemoteConfig, password: String) -> Result<Self> {
        ...
        let site_url = normalize_tenant_site(&config.site)?;

        // The output file is where the raw_cookie loader will read from later.
        let output = config.auth.raw_cookie_file.trim();
...
fn normalize_tenant_site(site: &str) -> Result<String> {
    let site = site.trim().trim_end_matches('/');
    let host = site.strip_prefix("https://").unwrap_or_default();
    if host.is_empty() || host.contains('/') || !host.ends_with(".atlassian.net") {
        return Err(MintError::Config(format!(
            "invalid site '{site}': expected https://<site>.atlassian.net"
        )));
    }
    Ok(site.to_string())
}
...
pub(crate) struct MintCookieArgs {
    #[arg(
        ...
        help = "OPTIONAL: config TOML path (reads [cookie], site,\n\
...
pub(super) fn run(args: &MintCookieArgs) -> AppResult<()> {
    let config = FsrtRemoteConfig::from_path(&args.config)?;
    ...
    if password.is_empty() {
        return Err(MintError::Config("ATL_PASSWORD is set but empty.".into()).into());
    }
...
async fn harvest_async(config: &HarvestConfig) -> Result<String> {
    ...
    println!("[+] Wrote {COOKIE_NAME} to {}", config.output.display());
    println!(
...
mod tests {
    use super::*;
    use forge_pen_test::{AuthConfig, CookieConfig};

    fn remote_config(output: &Path) -> FsrtRemoteConfig {
        FsrtRemoteConfig {
            site: "https://example.atlassian.net/".to_string(),
            auth: AuthConfig {
                raw_cookie_file: output.display().to_string(),
            },
            ...
            cookie: Some(CookieConfig {
                ...
                headed: Some(true),
                verify_wait_secs: Some(45),
            }),
        }
    }

    #[test]
    fn harvest_config_uses_shared_site_and_cookie_path() {
        let output = std::env::temp_dir().join("fsrt-cookie-test.txt");
        let config =
            HarvestConfig::from_remote_config(&remote_config(&output), "password".into()).unwrap();

        assert_eq!(config.site_url, "https://example.atlassian.net");
        assert_eq!(config.output, output);
        assert_eq!(config.username, "tester@example.com");
        assert!(config.headed);
        assert_eq!(config.verify_wait, Duration::from_secs(45));
    }

    #[test]
    fn tenant_site_rejects_non_tenant_urls() {
        for site in ["http://example.atlassian.net", "https://example.com", ""] {
            assert!(normalize_tenant_site(site).is_err(), "accepted {site}");
        }
    }
}
