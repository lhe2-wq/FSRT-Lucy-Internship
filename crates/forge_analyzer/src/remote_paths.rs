    ...
    for (_, block) in body.iter_blocks_enumerated() {
        for inst in &block.insts {
            ...
            if assigned.projections.is_empty()
                ...
                && let Some(found) = resolve_path_for_var(body, src_var, depth - 1)
            {
                return Some(found);
            }
        }
...
mod tests {
    ...
    use super::*;
    use crate::ir::{BasicBlockId, Variable};

    #[test]
    fn collects_literal_remote_key_and_local_path() {
        let options = VarId(1);
        let mut body = Body::new_for_test();
        body.blocks[BasicBlockId(0)].insts = vec![
            Inst::Assign(
                Variable {
                    base: Base::Var(options),
                    projections: smallvec![Projection::Known("path".into())],
                },
                Rvalue::Read(Operand::Lit(Literal::Str("/rest/api/3/myself".into()))),
            ),
            Inst::Expr(Rvalue::Intrinsic(
                Intrinsic::InvokeRemote,
                smallvec![
                    Operand::Lit(Literal::Str("backend".into())),
                    Operand::Var(Variable::from(options)),
                ],
            )),
        ];

        assert_eq!(
            collect_remote_paths(&body),
            vec![RemotePath {
                remote_key: Some("backend".to_string()),
                path: Some("/rest/api/3/myself".to_string()),
            }]
        );
    }

    #[test]
    fn reports_dynamic_options_without_dropping_the_call_site() {
        let mut body = Body::new_for_test();
        body.blocks[BasicBlockId(0)].insts = vec![Inst::Expr(Rvalue::Intrinsic(
            Intrinsic::InvokeRemote,
            smallvec![
                Operand::Lit(Literal::Str("backend".into())),
                Operand::Var(Variable::from(VarId(1))),
            ],
        ))];
